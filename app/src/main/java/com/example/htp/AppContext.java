...
//		 UIHelper.sendBroadcastForNotice(this);
    ...
    private void init() {
        // 初始化网络请求
        /*
         * AsyncHttpClient client = new AsyncHttpClient(); PersistentCookieStore
    ...
    private void initLogin() {
        SysAcct acct = getLoginSysAcct();
        if (null != acct && !acct.getId().equals("")) {
            login = true;
            loginUid = acct.getId();
        } else {
            this.cleanLoginInfo();
    ...
    public void saveUserInfo(final SysAcct acct) {
        this.loginUid = acct.getId();
        this.login = true;
        cleanLoginInfo();
        setProperties(new Properties() {
            {
                setProperty("acct.uid", acct.getId());
                setProperty("acct.phone", acct.getPhone());
                setProperty("acct.password", acct.getPassword());
     ...
     */
    @SuppressWarnings("serial")
    public void updateSysAcctInfo(final SysAcct acct) {
        setProperties(new Properties() {
            {
                setProperty("acct.uid", acct.getId());
                setProperty("acct.phone", acct.getPhone());
     ...
     * @return
     */
    public SysAcct getLoginSysAcct() {
        SysAcct acct = new SysAcct();
        acct.setId(getProperty("acct.uid"));
        acct.setPhone(getProperty("acct.phone"));
        acct.setPassword(getProperty("acct.password"));
        return acct;
    }
    ...
    public void cleanLoginInfo() {
        this.loginUid = "";
        this.login = false;
        removeProperty("acct.uid", "acct.password", "acct.phone"
        );
    }
    ...
    public void clearAppCache() {
		 ...
		 * DataCleanManager.cleanInternalCache(this); // 2.2版本才有将应用缓存转移到sd卡的功能
		 * if (isMethodsCompat(android.os.Build.VERSION_CODES.FROYO)) {
		 * DataCleanManager.cleanCustomCache(MethodsCompat
		 * .getExternalCacheDir(this)); } // 清除编辑器保存的临时内容 Properties props =
		 * getProperties(); for (Object key : props.keySet()) { String _key =
		 * key.toString(); if (_key.startsWith("temp")) removeProperty(_key); }
